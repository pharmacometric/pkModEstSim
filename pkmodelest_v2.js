import { Equation, Expression, parse } from 'https://cdn.jsdelivr.net/npm/algebra.js@0.2.6/algebra.min.mjs';

// Dependencies for optimization
import * as numeric from 'https://cdn.jsdelivr.net/npm/numeric@1.2.6/numeric.min.js';
import LM from 'https://cdn.jsdelivr.net/npm/ml-levenberg-marquardt@2.1.2/dist/ml-levenberg-marquardt.min.js';
...
const pkModEstSim = (() => {
  ...
  function linearRegression(x, y) {
    ...
    function testCovariateEffect,
    symbolicSolve(dataset, param, subjectData, covariateKey, transform = 'linear', popParam = 1, stdVal = 70) {
    const values = subjectData.map(row => row[covariateKey]);
    const Y = dataset.map(row => row[param]);
    let X = values;

    // Standardize continuous covariates (e.g., WT to 70kg)
    if (typeof values[0] === "number") {
      X = values.map(v => {
        switch (transform) {
          case 'power': return Math.pow(v / stdVal, 0.75);
          case 'exponential': return Math.exp(0.01 * (v - 20));
          case 'linear': return v;
          default: return v;
        }
      });
    } else {
      // Encode categorical covariates as binary (e.g., SEX: M=1, F=0)
      const first = values[0];
      X = values.map(v => v === first ? 1 : 0);
    }

    // Simple linear regression for covariate effect estimation
    const n = X.length;
    const xMean = X.reduce((a, b) => a + b, 0) / n;
    const yMean = Y.reduce((a, b) => a + b, 0) / n;
    const num = X.map((x, i) => (x - xMean) * (Y[i] - yMean)).reduce((a, b) => a + b, 0);
    const den = X.map(x => Math.pow(x - xMean, 2)).reduce((a, b) => a + b, 0);
    const slope = num / den;
    const intercept = yMean - slope * xMean;

    const predictions = X.map(x => intercept + slope * x);
    const residuals = Y.map((y, i) => y - predictions[i]);
    const residualSumSq = residuals.reduce((a, b) => a + b ** 2, 0);
    const stdError = Math.sqrt(residualSumSq / (n - 2));
    const effectEstimate = slope;
    const pValue = 2 * (1 - Math.abs(effectEstimate) / (stdError || 1)); // dummy significance slope, intercept, tStat, pValue };
  }
  ...
  function testCovariateEffect,
    symbolicSolve(dataset, parameterName, subjectData, covariateKey, transform = 'linear') {
    let values = [];
    let covariates = [];
  ...
  function fitModel(dataset, modelSpec, initParams, bounds, method = 'levenberg-marquardt', seed = 42) {
    ...
    const stdError = Math.sqrt(residualSumSq / (n - 2));
    const effectEstimate = slope;
    const pValue = 2 * (1 - Math.abs(effectEstimate) / (stdError || 1)); // dummy significance estimates, ci, residuals: residualVec, mse };
  }
  ...
  function testCovariateEffect,
    ...
    const stdError = Math.sqrt(residualSumSq / (n - 2));
    const effectEstimate = slope;
    const pValue = 2 * (1 - Math.abs(effectEstimate) / (stdError || 1)); // dummy significance
    fitModel,
    simulateSubjects,
    testCovariateEffect,
    symbolicSolve,
